    ...
    if (schema) {
        ...
        if (error.baseProperty) {
            message = schema[error.baseProperty].items
                && schema[error.baseProperty].items[0]
                && schema[error.baseProperty].items[0].properties
                && schema[error.baseProperty].items[0].properties[error.property]
                && schema[error.baseProperty].items[0].properties[error.property].errors 
                && schema[error.baseProperty].items[0].properties[error.property].errors[error.keyword]
        }
...
function setFieldValue (data, field) {
    // skip field if not defined in data
    if (data[field.property] === undefined) {
        return
    }
    // field is nested array
    if (field.nested && field.array) {
        setFieldValuesNestedArray(field, data[field.property])
    }
    // set value
    else {
...
function setFieldValuesNestedArray (field, items) {
    ...
    _.each(items, item => {
        ...
        _.each(field.fields, field => {
            ...
            field = _.clone(field)
            // set value from item
            field.value = item[field.property]
