...
function initRecord (record) {
    ...
    this.setFieldValues(record.data, true)
}
 ...
 * @throws {Error}
 */
function setFieldValues (data, isRecord) {
    // skip if no data
    if (!data) {
        return
    }
    ...
    _.each(this.fields, field => {
        ...
        if (Array.isArray(field)) {
            _.each(field, field => this.setFieldValue(data, field, isRecord))
        }
        // set value for field
        else {
            this.setFieldValue(data, field, isRecord)
        }
    })
}
 ...
 * @throws {Error}
 */
function setFieldValue (data, field, isRecord) {
    // do not set value for password input
    if (field.inputType === 'password') {
        return
    }
    // get value using either property or name fields
    var value = defined(data[field.property]) ? data[field.property] : data[field.name]
    // data is a record
    if (data.ImmutableCoreModelRecord) {
        // set record for field
        field.record = data
        // value was not found - could be due to resolving record so try to
        // lookup by default id column names
        if (!defined(value)) {
            // get default id column name from model
            var defaultIdColumnName = field.record.model.defaultColumns[field.property]
            // use for value if found
            if (defined(defaultIdColumnName)) {
                value = data[defaultIdColumnName]
            }
        }
    }
    ...
    if (isRecord && field.immutable && defined(field.value)) {
        field.readonly = true
    }
}
