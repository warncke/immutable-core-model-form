...
function buildArrayField (args, field, form, model, schema) {
    ...
    assert.ok(Array.isArray(schema.items) && schema.items.length === 1 && schema.items[0] && typeof schema.items[0] === 'object', field.property+' array must have single object for items')
    // get schema for sub object
    var subSchema = schema.items[0]
    // require sub schema to be object with properites
    assert.ok(subSchema.type === 'object' && subSchema.properties && typeof subSchema.properties === 'object', field.property+' array item must be object with properties')
    // get data properites from schema
